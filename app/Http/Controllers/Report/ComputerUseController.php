...
{
    /**
     * Display the online research (computer use) report page.
     */
    ...
    /**
     * Export computer use report as PDF.
     */
    ...
    /**
     * Return JSON aggregation for graphs.
     */
    ...
    private function generateData(Request $request, Log $model, bool $isExport = false)
    {
        ...
        $query = $model->newQuery()
            ->select(['id', 'user_id', 'time_in', 'remarks'])
    ...
    private function generatePDF(Collection $data): void
    {
        ...
        $dompdf->render();
        $dompdf->stream('computer-use-report '.date('Y-m-d').'.pdf', ['Attachment' => true]);
        exit;
    }
