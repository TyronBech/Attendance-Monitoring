...
// use simple navigation instead of importing @inertiajs/inertia to avoid resolving issues
...
export default function UserLogs() {
    ...
    useEffect(() => {
        // fetch aggregated graph data
        const params = new URLSearchParams();
        if (start) params.set('start', start);
        if (end) params.set('end', end);
        fetch(routes.userLogsGraph() + '?' + params.toString())
            .then((r) => r.json())
            .then(setPoints)
            .catch(() => setPoints([]));
    }, [start, end]);
    ...
    function applyFilters(e) {
        e && e.preventDefault();
        window.location.href = routes.userLogs() + `?start=${start}&end=${end}`;
    }
    ...
    return (
        <div>
            ...
            >
                ...
                <a
                    href={routes.userLogsExport({ format: 'csv', start, end })}
                    style={{ marginLeft: 12 }}
                >
                    Export CSV
                </a>
